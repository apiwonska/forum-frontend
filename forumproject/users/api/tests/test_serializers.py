...
class RegistrationSerializerTestCase(BaseSerializerTestCase):
    ...
    def test_password_is_not_in_data(self):
        ...
        serializer.is_valid()
        self.assertEqual(
            serializer.data, {"username": "NewTestUser", "email": "new.user@test.com"}
        )
    ...
    def test_validate_email_unique(self):
        ...
        self.assertEqual(
            str(serializer.errors["email"][0]), "This field must be unique."
        )
    ...
    def test_validate_username_length(self):
        ...
        self.assertEqual(
            ...
            "Username must be at least 3 characters length.",
        )
    ...
    def test_raise_error_when_passwords_dont_match(self):
        ...
        self.assertFalse(is_valid)
        self.assertEqual(
            str(serializer.errors["password2"][0]), "Passwords must match."
        )
    ...
    def test_raise_error_when_password_is_not_valid(self):
        ...
        self.assertFalse(is_valid)
        self.assertEqual(
            str(serializer.errors["password"][0]), "This password is too common."
        )
...
class ChangePasswordSerializerTestCase(BaseSerializerTestCase):
    ...
    def test_updates_user_password(self):
        ...
        serializer = ChangePasswordSerializer(
            instance=self.user, data=self.correct_data, partial=True
        )
        is_valid = serializer.is_valid()
        serializer.save()
